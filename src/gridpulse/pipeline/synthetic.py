"""Deterministic synthetic electricity + weather payloads for offline runs.

This module provides synthetic fixture generation for running the GridPulse
pipeline, forecasting, and dispatch modules without live ENTSO-E data.

IMPORTANT: Everything here is a **hand-crafted synthetic fixture** that exists
only to exercise the real pipeline / forecast code paths offline. It is NEVER
real, downloaded, or representative data, and it must never be combined with
live data in a way that could be mistaken for a real historical dataset. Any
benchmark produced from these payloads is labelled ``FIXTURE-VERIFIED``.
...
def _generation_xml(start: datetime, end: datetime, per_hour: list[dict]) -> bytes:
    ...
    for ts_id, values in blocks:
        ...
        # Inject the PSR element right after the curveType line.
        raw = raw.replace(
            "      <curveType>A01</curveType>\n",
            "      <curveType>A01</curveType>\n      <MktPSRType>\n        <psrType>"
            + _PSR_FOR_TS[ts_id]
