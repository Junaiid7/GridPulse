...
def test_dst_spring_forward() -> None:
    ...
    assert utc_offset_minutes(datetime(2024, 3, 31, 0, 30, tzinfo=UTC)) == CET_OFFSET_MIN
    assert utc_offset_minutes(datetime(2024, 3, 31, 1, 30, tzinfo=UTC)) == CEST_OFFSET_MIN


def test_dst_fall_back() -> None:
    # 2024-10-27: at 01:00 UTC Amsterdam drops CEST -> CET.
    assert utc_offset_minutes(datetime(2024, 10, 27, 0, 30, tzinfo=UTC)) == CEST_OFFSET_MIN
    assert utc_offset_minutes(datetime(2024, 10, 27, 1, 30, tzinfo=UTC)) == CET_OFFSET_MIN


def test_local_date_across_midnight() -> None:
    assert local_date(datetime(2024, 1, 1, 23, 0, tzinfo=UTC)) == datetime(2024, 1, 2).date()


def test_local_hour_boundary() -> None:
    assert local_hour(datetime(2024, 7, 1, 0, 0, tzinfo=UTC)) == 2  # UTC midnight -> 02:00 CEST
    assert local_hour(datetime(2024, 1, 1, 23, 0, tzinfo=UTC)) == 0  # UTC 23:00 -> 00:00 CET next day