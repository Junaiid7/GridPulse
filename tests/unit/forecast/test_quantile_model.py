...
from gridpulse.forecast.models import LinearRegressionModel, QuantileRegressionModel
...
def test_fit_and_predict_produce_ordered_quantile_triples(feature_rows):
    ...
    rows = [r for r in ds.rows if split.validation_start <= r.issue_time < split.validation_end]
...
def test_recovers_linear_relation_in_sample():
    ...
    model = QuantileRegressionModel(feature_columns=["x_feat"], random_state=0, n_estimators=120)
    ...
    evals = [(p, r) for p, r in zip(preds, [r for r in ds.rows if lo <= r.issue_time < mid])
             if p.has_quantiles]
    if not evals:
...
def test_missing_cells_yield_none_never_filled():
    ...
    rows = _linear_rows(30)
    ds = build_forecasting_dataset(rows)
    split = chronological_split_by_fraction(ds.issue_times, train_fraction=0.7, validation_fraction=0.15)
    ...
    val_rows = [r for r in ds.rows if split.validation_start <= r.issue_time < split.validation_end]
    assert val_rows
...
def test_too_few_rows_means_unfitted_and_all_none():
    ...
    preds = model.predict(ds, start=ds.rows[0].issue_time, end=ds.rows[-1].issue_time + timedelta(hours=1))
...
def test_determinism_under_fixed_seed(feature_rows):
    ...
    p1 = [(p.p10, p.p50, p.p90) for p in m1.predict(ds, start=split.test_start, end=split.test_end)]

    m2 = QuantileRegressionModel(feature_columns=cols, random_state=11)
    m2.fit(ds, start=split.train_start, end=split.train_end)
    p2 = [(p.p10, p.p50, p.p90) for p in m2.predict(ds, start=split.test_start, end=split.test_end)]
    assert p1 == p2
...
def test_save_load_round_trip(feature_rows, tmp_path):
    ...
    a = [(p.p10, p.p50, p.p90) for p in model.predict(ds, start=split.test_start, end=split.test_end)]
    b = [(p.p10, p.p50, p.p90) for p in loaded.predict(ds, start=split.test_start, end=split.test_end)]
    assert a == b
